...
import os
os.environ.setdefault("MUJOCO_GL", "osmesa")  # must be set before mujoco/metaworld import
...
async def evo1_infer(
    ...
    if isinstance(resp, dict) and "error" in resp:          # fix: surface server-side errors
        raise RuntimeError(f"Server inference error: {resp['error']}")
        ...
        for idx in ordered:
            ...
            for ep in range(args.episodes):
                ...
                while steps < args.horizon and not done:
                    ...
                    try:                              # fix: don't let one inference error kill the run
                        actions = await evo1_infer(ws, img_bgr, state_history, prompt)
                    except RuntimeError as e:
                        log(f"[Task {idx:02d} ep{ep+1}] Inference error: {e} — skipping episode")
                        break
