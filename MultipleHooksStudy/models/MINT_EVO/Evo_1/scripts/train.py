...
def load_checkpoint(model_engine, load_dir, accelerator, tag, resume_pretrain=False, model_only=False):
    ...
    if model_only:
        ...
        if accelerator.is_main_process:
            logging.info(f"Loaded model weights only from {checkpoint_dir}")
    else:
        accelerator.load_state(checkpoint_dir)
        if accelerator.is_main_process:
            logging.info(f"Loaded full state from {checkpoint_dir}")

    meta_path = os.path.join(checkpoint_dir, "meta.json")
    if os.path.exists(meta_path):
        with open(meta_path) as f:
            meta = json.load(f)
        step = 0 if resume_pretrain else int(meta.get("step", 0))
        return step, meta
    return 0, {}
...
def train(config):
    ...
    global accelerator
    accelerator = Accelerator(mixed_precision="bf16")  # ← change 1
    ...
    # === Scheduler — must be created and registered BEFORE load_state ===
    scheduler = LambdaLR(optimizer, get_lr_lambda(warmup_steps, max_steps, resume_step=0))
    accelerator.register_for_checkpointing(scheduler)  # ← change 2
    ...
    if resume:
        ...
        step, client_state = load_checkpoint(
            model_engine,
            load_dir=resume_dir,
            accelerator=accelerator,
            tag=resume_tag,
