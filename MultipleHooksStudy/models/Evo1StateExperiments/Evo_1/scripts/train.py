...
def save_checkpoint(save_dir, step, model_engine, loss, accelerator, config=None, norm_stats=None):
    ...
    # === Old DeepSpeed checkpoint save (commented out) ===
    # client_state = {
    #     "step": step,
...
def load_checkpoint_with_deepspeed(model_engine, load_dir, accelerator, tag="step_best", load_optimizer_states=True, resume_pretrain=False):  # noqa: model_engine kept for API compat
    ...
    # === Old DeepSpeed checkpoint load (commented out) ===
    # try:
...
def train(config):
    ...
    if not get_with_warning(config, "skip_pretrain", False):
        ...
        if state_encoder is not None:
            ...
            decoder = pretrain_phase0(
                ...
                accelerator=accelerator,
                config={
                    "pretrain_steps": get_with_warning(config, "pretrain_steps", 1000),
                    "pretrain_lr":    get_with_warning(config, "pretrain_lr", 1e-3),
                    "lambda_orth":    get_with_warning(config, "lambda_orth", 0.01),
                    "log_interval":   get_with_warning(config, "log_interval", 50),
                    "state_dim":      get_with_warning(config, "state_dim", 7),
    ...
    while step < max_steps:
        for batch in tqdm(dataloader, desc="Training", disable=not accelerator.is_main_process):
            ...
            # states should be [B, k+1, state_dim] if dataset supports history
            # falls back to [B, state_dim] safely via ndim guard in flow_matching.py
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embedding_strain", type=str, default="C", choices=["none", "A", "B", "C"],
                        help="Type embedding strategy. none=Exp1, A=random, B=random+orth, C=orth init frozen.")
    parser.add_argument("--features", type=str, nargs="+",
                        default=["position", "velocity", "acceleration", "trace", "deviation"],
                        help="Which features to encode from state history.")
    parser.add_argument("--trace_decay", type=float, default=0.9,
                        help="Fixed decay scalar rho for eligibility trace.")
    parser.add_argument("--lambda_orth", type=float, default=0.01,
                        help="Weight for orthogonality loss in Phase 0 (Strain B only).")
    parser.add_argument("--pretrain_steps", type=int, default=1000,
                        help="Number of Phase 0 state encoder pretraining steps.")
